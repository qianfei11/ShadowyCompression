...
public class ConvertColorspace {
	...
	public static BigInteger[][][][] convertRGB2YUV(BigInteger[][][][] Images, PublicKey PK, BigInteger sk) {
		...
		for (int i = 0; i < Images.length; i++) {
			for (int m = 0; m < 8; m++) {
				for (int n = 0; n < 8; n++) {
					...
					out[i][1][m][n] = Cb;
					out[i][2][m][n] = Cr;
//					BigInteger dY = p.De(PK, sk, Y);
//					BigInteger dCb = p.De(PK, sk, Cb);
//					BigInteger dCr = p.De(PK, sk, Cr);
//					BigInteger thresholdVal = new BigInteger(String.valueOf((int)Math.pow(2, 22)));
//					if (dY.compareTo(thresholdVal) == 1) {
//						dY = dY.subtract(PK.n);
//					}
//					if (dCb.compareTo(thresholdVal) == 1) {
//						dCb = dCb.subtract(PK.n);
//					}
//					if (dCr.compareTo(thresholdVal) == 1) {
//						dCr = dCr.subtract(PK.n);
//					}
//					out[i][0][m][n] = dY.divide(new BigInteger("10000"));
//					out[i][1][m][n] = dCb.divide(new BigInteger("10000"));
//					out[i][2][m][n] = dCr.divide(new BigInteger("10000"));
				}
			}
		}
